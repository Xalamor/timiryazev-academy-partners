...
•  .gitignore  
  Служебный файл Git: перечисляет файлы/папки, которые не должны попадать в репозиторий (например, node_modules, файлы сборки и т.п.).
...
•  зависимости: next, react, react-dom, leaflet, react-leaflet, axios, tailwindcss, typescript и др.
•  package-lock.json  
  Зафиксированные версии всех зависимостей.

•  tsconfig.json  
  Настройки TypeScript:
•  включает .ts, .tsx и некоторые служебные типы .next/...;
...
•  next.config.ts  
  Конфигурация Next.js (сейчас содержит пустой объект, может расширяться при необходимости).

•  eslint.config.mjs  
  Конфигурация ESLint на основе eslint-config-next:
•  подключены правила core-web-vitals и typescript;
•  игнорируются .next, out, build и next-env.d.ts.
•  postcss.config.mjs  
  Конфигурация PostCSS:
•  используется плагин @tailwindcss/postcss (Tailwind CSS 4).
•  README.md  
  Стандартная документация от create-next-app (можно заменить на этот документ).
...
◦  <main> для рендеринга содержимого страниц.
...
•  Главная страница — интерактивная карта партнёров.
...
•  Стили для ссылок (a без подчёркивания).

app/partners.ts

Файл с данными партнёров и вспомогательными функциями:
...
•  адаптивной вёрстки для мобильных устройств.

app/partners/[country]/page.tsx

Динамическая страница /partners/[country]:

•  Помечена "use client", так как использует React‑hooks.
•  Использует:
...
•  Содержит ссылку «Смотреть всех партнеров →» на /partners.
...
•  общего контейнера страницы и хедера с описанием;
...
•  Вёрстка через inline‑стили + небольшой блок <style jsx> для адаптивности:
...
•  "use client", так как использует React‑хуки и динамические импорты.
•  Использует:
...
•  Логика:
◦  Загружает GeoJSON‑данные стран мира через fetch из публичного GitHub‑репозитория.
◦  Для каждой страны:
...
•  адаптивные правила для десктопа (горизонтальное расположение элементов).
...
2. В массиве PARTNERS добавьте новый объект формата:
   {
     id: "unique-id",               // уникальный идентификатор (строка без пробелов)
     name: "Название университета", // отображаемое имя
     country: "CountryName",        // страна (должна совпадать с названиями в PARTNER_COUNTRIES/GeoJSON)
     city: "Город",
     description: "Краткое описание сотрудничества",
     year: 2020,                    // год начала сотрудничества
     programs: [
       "Программа 1",
       "Программа 2",
     ],
     website: "https://example.com", // (опционально)
     studentsCount: 50,              // (опционально) количество студентов по обмену
     // logo: "/logos/example.svg",  // (опционально, сейчас не используется)
   }

    Убедитесь, что:
◦  id уникален среди всех партнёров (используется как id DOM‑элемента и в ссылках вида #id);
◦  country написана точно так же, как в других местах (регистр и пробелы важны). Примеры:
...
4. Если страна уже есть в PARTNER_COUNTRIES, переходите к следующему шагу.  
   Если страна новая, смотрите раздел «Как добавить новую страну».
...
5. В PARTNER_DATA найдите соответствующий ключ страны и добавьте название нового университета в массив значений.  
   ...
   Пример для Германии:
     ...
     Germany: [
       ...
       "Новый университет-партнёр", // добавили
     ],
     France: [/* ... */],
     // ...
   };
   Сохраните файл. После перезапуска dev‑сервера (npm run dev) новый партнёр появится:
...
▪  либо добавьте новую страну (см. следующий раздел).
...
◦  учтите, что тогда страна перестанет подсвечиваться на карте и исчезнет из списков.
...
1. В PARTNERS добавьте партнёра с country: "Italy" (см. пример объекта выше).
2. В PARTNER_COUNTRIES добавьте новую страну:
   ...
   В PARTNER_DATA добавьте ключ:
      export const PARTNER_DATA: Record<string, string[]> = {
     Germany: [/* ... */],
     France: [/* ... */],
     // ...
     Italy: [
       "Название итальянского университета",
     ],
   };
   Важно: название "Italy" должно совпадать:
◦  в PARTNER_COUNTRIES;
